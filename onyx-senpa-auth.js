...
(function () {
  ...
  function injectUI() {
    ...
    style.textContent = [
      '#onyx-auth-fab{position:fixed;top:30px;left:8px;right:auto;transform:none;z-index:2147483000;display:flex;align-items:center;gap:8px;padding:8px 12px;border:0;border-radius:8px;background:#12151c;color:#e8ecf3;font:600 12px/1.2 Segoe UI,system-ui,sans-serif;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.35)}',
    ...
    function positionFabBesideLeaderboard() {
      var leaderboard = document.getElementById('leaderboard-hud');
      if (!leaderboard) return;
      var rect = leaderboard.getBoundingClientRect();
      var gap = 10;
      var nextLeft = rect.left - fab.offsetWidth - gap;
      fab.style.left = Math.max(8, nextLeft) + 'px';
      fab.style.right = 'auto';
      fab.style.top = Math.max(8, rect.top) + 'px';
      fab.style.transform = 'none';
    }
    positionFabBesideLeaderboard();
    window.addEventListener('resize', positionFabBesideLeaderboard);
    setTimeout(positionFabBesideLeaderboard, 250);
    setTimeout(positionFabBesideLeaderboard, 1000);
    try {
      var lbObserver = new MutationObserver(positionFabBesideLeaderboard);
      var lb = document.getElementById('leaderboard-hud');
      if (lb) lbObserver.observe(lb, { childList: true, subtree: true, characterData: true });
    } catch (_) {}
