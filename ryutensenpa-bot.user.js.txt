// ==UserScript==
// @name         Ryuten Senpa Bot Panel
// @namespace    https://github.com/Shrkawyy/ryutensenpa
// @version      6.1.13
// @description  Loads the JaxxV6 client and bot control panel on Senpa.
// @author       Shrkawyy
...
// @grant        GM_info
// @connect      ryutensenpa.vercel.app
// @noframes
// @updateURL    https://ryutensenpa.vercel.app/ryutensenpa-bot.user.js
// @downloadURL  https://ryutensenpa.vercel.app/ryutensenpa-bot.user.js
// ==/UserScript==

(function () {
  'use strict';

  const DEFAULT_BASE_URL = 'https://ryutensenpa.vercel.app/ffa/';
  const CLIENT_FILE = 'client.html';
  const VERSION = '6.1.13';
  const BASE_URL_KEY = 'ryutensenpa:base-url';
  ...
  GM_xmlhttpRequest({
    ...
    async onload(response) {
      if (response.status < 200 || response.status >= 300) {
        renderError(`The Ryuten server returned HTTP ${response.status} for ${CLIENT_FILE}.`);
        return;
      }
      try {
    ...
    ontimeout() {
      renderError('The Ryuten server request timed out.');
    },
    onerror() {
      renderError('The Ryuten server request failed.');
    }
  });
})();
